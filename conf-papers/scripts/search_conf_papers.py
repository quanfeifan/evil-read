...
from search_arxiv import (
    ...
    S2_RATE_LIMIT_WAIT,
    PREFERENCE_MIN_FEEDBACK,
    PREFERENCE_MAX_WEIGHT,
    PREFERENCE_RAMP_FEEDBACK,
)
...
def filter_and_score_papers(papers: List[Dict], cp_config: Dict, top_n: int = 10, preferences: Optional[Dict] = None) -> List[Dict]:
    """
    ...
    for paper in papers:
        ...
        weights = dict(WEIGHTS_CONF)
        pref_score = None

        if _calc_pref and pref_weight > 0:
            pref_score = _calc_pref(paper, preferences)
            normalized['preference'] = (pref_score / SCORE_MAX) * 10
            scale = 1.0 - pref_weight
            weights = {k: v * scale for k, v in weights.items()}
            weights['preference'] = pref_weight
...
def main():
    ...
    top_papers = filter_and_score_papers(filtered_papers, cp_config, top_n=top_n, preferences=user_preferences)
